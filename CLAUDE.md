...
| Pass Rate | 100% (Chromium QA verified) | 2026-05-15 |
...
| Browsers | 1 (Chromium) - Firefox, WebKit removed for efficiency | 2026-05-15 |
...
**Company:** Bermann (<https://www.bermann.cl/soluciones>)
...
├── AGENTS.md                   # Skills system index + auto-invoke rules
├── CLAUDE.md                   # This file (project documentation)
├── GEMINI.md                   # Same documentation (mirror of CLAUDE.md)
...
│   ├── tms-page-objects/       # Page Object Model template
│   ├── tms-tests/              # Test structure and phases
│   ├── tms-data/               # Data generation (RUT, factories)
│   ├── tms-atomic-e2e/         # Atomic E2E test patterns (NEW)
│   ├── tms-ultimamilla/        # Última Milla automation (NEW)
│   ├── jql-tickets/            # JQL for ready-to-test tickets
│   ├── jira-ticket-writer/     # Jira User Stories & subtasks
│   └── ...                     # See AGENTS.md for full list
├── src/
│   ├── modules/                # Domain-Driven Architecture (9 modules)
...
│   │   ├── finanzas/           # Prefactura (E2E atómico)
│   │   ├── ultimamilla/        # Pedidos, Asignación, Monitoreo UM (4 Page Objects)
│   │   └── configAdmin/        # Config: UnidadNegocio, TipoOperacion, TipoServicio, etc. (8 Page Objects)
...
│   │   │   ├── 01-entidades/   # Entity creation tests (transportista, cliente, etc.)
...
│   ├── api-helpers/            # TmsApiClient, helpers, DataPathHelper, OperationalDataLoader
│   ├── helpers/
│   │   └── auth.setup.ts       # Global authentication setup
│   ├── experiments/            # Experimental test scripts
│   └── exploration/            # Page inspection scripts
├── scripts/                    # Utility scripts (pw:run wrapper, CI validation, engram, SDD sync)
│   ├── run-playwright-suite.mjs    # Main execution wrapper
│   ├── ci/validate-workflow-scripts.mjs  # CI preflight validation
│   ├── engram/                 # Engram persistence scripts
│   └── sdd/                    # SDD sync scripts
...
├── .atl/                       # Auto-generated: skill-registry.md
...
### 5. Unified Data Persistence

**Innovation:** The framework uses unified JSON files optimized for Chromium-only execution.
...
playwright/.auth/user.json
     |
base-entities.setup.ts (Chromium only)
     |
...
**Note:** Firefox and WebKit were removed. Only Chromium is active.
...
All tests run through the wrapper `scripts/run-playwright-suite.mjs` (`npm run pw:run`) which handles run locks, env-based project selection, and cleanup.
...
npm run sdd:sync:pull                   # Pull latest SDD artifacts (openspec/)
...
- `playwright.yml` — Legacy single test runner (removed in favor of consolidated PR pipeline)
