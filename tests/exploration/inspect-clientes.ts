...
async function inspectClientesPage() {
  const browser = new BrowserManager({ headless: false });

  try {
    logger.info('='.repeat(60));
    logger.info('🔍 Starting Clientes Page Inspection');
    logger.info('='.repeat(60));
    ...
    // ========================================
    // PHASE 2: Navigate to Cliente/Crear
    // ========================================
    logger.info('\n🧭 PHASE 2: Navigate to Clientes/Crear');
    await page.goto('https://moveontruckqa.bermanntms.cl/clientes/crear');
    ...
    if (!currentUrl.includes('/clientes/crear')) {
      throw new Error('Failed to navigate to clientes/crear page');
    }

    logger.info('✅ Successfully navigated to Cliente Crear page');
    ...
    console.log('\n' + '='.repeat(60));
    console.log('📋 CLIENTE PAGE SELECTORS (JSON)');
    ...
    await page.screenshot({
      path: './reports/screenshots/clientes-crear-inspection.png',
      fullPage: true
    });
    logger.info('✅ Screenshot saved: ./reports/screenshots/clientes-crear-inspection.png');
    ...
    try {
      const page = browser.getPage();
      await page.screenshot({
        path: `./reports/screenshots/clientes-inspection-error-${Date.now()}.png`,
        fullPage: true
      });
