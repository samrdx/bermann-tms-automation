...
async function inspectViajesPage() {
  const browser = new BrowserManager({ headless: false });

  try {
    logger.info('='.repeat(60));
    logger.info('🔍 Starting Viajes Page Inspection');
    logger.info('='.repeat(60));
    ...
    // ========================================
    // PHASE 2: Navigate to Viajes/Crear
    // ========================================
    logger.info('\n🧭 PHASE 2: Navigate to Viajes/Crear');
    ...
    if (!currentUrl.includes('/viajes/crear')) {
      throw new Error('Failed to navigate to viajes/crear page');
    }

    logger.info('✅ Successfully navigated to Planificar Viajes page');
    ...
    console.log('\n' + '='.repeat(60));
    console.log('📋 VIAJES PAGE SELECTORS (JSON)');
    ...
    await page.screenshot({
      path: './reports/screenshots/viajes-crear-inspection.png',
      fullPage: true
    });
    logger.info('✅ Screenshot saved: ./reports/screenshots/viajes-crear-inspection.png');
    ...
    try {
      const page = browser.getPage();
      await page.screenshot({
        path: `./reports/screenshots/viajes-inspection-error-${Date.now()}.png`,
        fullPage: true
      });
