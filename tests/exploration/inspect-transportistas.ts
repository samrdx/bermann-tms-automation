...
async function inspectTransportistasPage() {
  const browser = new BrowserManager({ headless: false });

  try {
    logger.info('='.repeat(60));
    logger.info('🔍 Starting Transportistas Page Inspection');
    logger.info('='.repeat(60));
    ...
    // ========================================
    // PHASE 2: Navigate to Transportista/Crear
    // ========================================
    logger.info('\n🧭 PHASE 2: Navigate to Transportistas/Crear');
    ...
    if (!currentUrl.includes('/transportistas/crear')) {
      throw new Error('Failed to navigate to transportistas/crear page');
    }

    logger.info('✅ Successfully navigated to Transportista Crear page');
    ...
    console.log('\n' + '='.repeat(60));
    console.log('📋 TRANSPORTISTA PAGE SELECTORS (JSON)');
    ...
    await page.screenshot({
      path: './reports/screenshots/transportistas-crear-inspection.png',
      fullPage: true
    });
    logger.info('✅ Screenshot saved: ./reports/screenshots/transportistas-crear-inspection.png');
    ...
    try {
      const page = browser.getPage();
      await page.screenshot({
        path: `./reports/screenshots/transportistas-inspection-error-${Date.now()}.png`,
        fullPage: true
      });
