...
async function inspectVehiculosPage() {
  const browser = new BrowserManager({ headless: false });

  try {
    logger.info('='.repeat(60));
    logger.info('🔍 Starting Vehículos Page Inspection');
    logger.info('='.repeat(60));
    ...
    // ========================================
    // PHASE 2: Navigate to Vehiculo/Crear
    // ========================================
    logger.info('\n🧭 PHASE 2: Navigate to Vehiculos/Crear');
    await page.goto('https://moveontruckqa.bermanntms.cl/vehiculos/crear');
    ...
    if (!currentUrl.includes('/vehiculos/crear')) {
      throw new Error('Failed to navigate to vehiculos/crear page');
    }

    logger.info('✅ Successfully navigated to Vehículo Crear page');
    ...
    console.log('\n' + '='.repeat(60));
    console.log('📋 VEHÍCULO PAGE SELECTORS (JSON)');
    ...
    await page.screenshot({
      path: './reports/screenshots/vehiculos-crear-inspection.png',
      fullPage: true
    });
    logger.info('✅ Screenshot saved: ./reports/screenshots/vehiculos-crear-inspection.png');
    ...
    try {
      const page = browser.getPage();
      await page.screenshot({
        path: `./reports/screenshots/vehiculos-inspection-error-${Date.now()}.png`,
        fullPage: true
      });
